...
/**
 * This ViewModel contains the data needed to be displayed on the config activity
 * for the widget.
 */

@UiThread
public class IngredientsWidgetViewModel extends AndroidViewModel {
    private final BakingRepository repository;
    private final LiveData<RecipeList> liveRecipeList;

    public IngredientsWidgetViewModel(Application application) {
        super(application);
    ...
    public void setWidgetRecipe(int widgetId, int recipeId) {
        repository.setWidgetRecipe(widgetId, recipeId);
    }
}
